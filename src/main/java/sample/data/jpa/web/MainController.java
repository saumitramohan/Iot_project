...
import sample.data.jpa.service.SendEmailAlert;
...
public class MainController {
    ...
    public ResponseEntity<User> addNewUser(@PathVariable("username") String username, @RequestBody(required = false) UserRequestBody userRequestObject) {
        ...
        if (userRequestObject != null) {
            ...
            user.setEmail(userRequestObject.getEmailId());
    ...
    public @ResponseBody
    Iterable<MetricEntityWrapper> getIndividualMetrics(@PathVariable("metricdescription") String metricdescription) {
    ...
    public @ResponseBody
    Iterable<MetricsWrapper> getUserHistoricalData(@PathVariable("deviceid") String deviceId) {
        ...
        User user = userRepository.findAllByDeviceId(deviceId);
        ...
        for (Metrics metric : metrics) {
            ...
            if (meticsWrapperObject.isAlertFlag()) {
                emailFlag = true;
            }
            alertList.add(meticsWrapperObject.getAlertValue());
            MetricsWrapperList.add(meticsWrapperObject);
        }
        Collections.sort(MetricsWrapperList);
        if (emailFlag) {
            SendEmailAlert ea = new SendEmailAlert(user.getEmail(), alertList);
