...
//	}
    ...
    public @ResponseBody Iterable<MetricsWrapper> getUserHistoricalData(@PathVariable("deviceid") String deviceId){
	    ...
	    for (Metrics metric : metrics){
            ...
            // Store last 10 values // check whether they are below thershold, trigger an email alert.
            if(metric.getMetricType() == 2){
                int counter =0 ;
                Iterable <Metrics> alertData = metricsRepository.findTop10ByMetricTypeOrderByEntryTimeStampDesc(metric.getMetricType());
                for(Metrics ad : alertData){
                    if(ad.getMetricValue()>120){
                        counter ++ ;
                    }
                }
                if(counter == 10){
